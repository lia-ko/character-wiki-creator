<script>
  ...
  function sampleFor(s){
    ...
    switch (s.type){
      ...
      case 'clocks': return [{ label: 'The ritual completes', segments: 6, filled: 2 }, { label: 'Suspicion rises', segments: 4, filled: 1 }];
      case 'npcroster': return [{ name: 'Mira Vos', role: 'tavern keeper', disp: 'friendly', want: 'to pay off her debt', quirk: 'wipes the same mug when nervous', where: 'The Gilded Eel', secret: 'launders coin for the guild', targetId: '', img: '' }, { name: 'Sergeant Kord', role: 'watch officer', disp: 'wary', want: 'a quiet ward', quirk: 'clipped, formal speech', where: 'East gate', secret: '', targetId: '', img: '' }];
